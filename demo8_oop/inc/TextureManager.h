...
class TextureManager
{
    public:
        ...
        void draw(std::string id, vec pos, uint16_t width, uint16_t height, SDL_RendererFlip flip = SDL_FLIP_NONE);
        void drawRotat(std::string id, vec pos, int width, int height, int currentRow, int currentFrame, double angle, SDL_RendererFlip flip = SDL_FLIP_NONE);
        void drawFrame(std::string id, vec pos, uint16_t width, uint16_t height, uint8_t currentRow, uint8_t currentFrame, int alpha = 255, SDL_RendererFlip flip = SDL_FLIP_NONE);
        ...
        void drawHealthBar(vec pos, int width, int height, int currentHP, int maxHP);
